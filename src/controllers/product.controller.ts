...
export class ProductController implements interfaces.Controller {
    ...
    public async getByName(
      ...
      let products: IProduct= await this.service.getByName(name);
      let productsReponse: ProductResponse=new ProductResponse(products);
    ...
    public async getByDescription(
      ...
      @response() response: express.Response) {
      let products: IProduct= await this.service.getByDescription(description);
      let productsReponse: ProductResponse=new ProductResponse(products);
      response.send(productsReponse);
    }
