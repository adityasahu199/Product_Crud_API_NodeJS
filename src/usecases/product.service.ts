...
export class ProductService {
    ...
    public async getByName(name: string): Promise<IProduct> {
      ...
      .catch((err) => {
        throw new BusinessException("R-002", "Error to get the product.");
      });
    }

    public async getByDescription(description: string): Promise<IProduct> {
      return this.gateway.findByDescription(description)
      .then((data: IProduct) => {
        return data;
      })
      .catch((err) => {
        throw new BusinessException("R-003", "Error to get the product.");
      });
    }
