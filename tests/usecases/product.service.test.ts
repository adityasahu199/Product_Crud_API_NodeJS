...
describe("Unit test to cover Product UC.", async () => {
    ...
    it("Should create a product sucessfully", async () => {
        ...
        expect("good product").to.be.eq(productinserted.description);
    ...
    it("Should get a product by name", async () => {
        // Given
        let product: IProduct = newProductInserted(newProduct(), "P-001");

        // Prepare
        when(gateway.findByName(product.name)).thenResolve(product);
        
        // When
        await productService.getByName(product.name).then((data: IProduct) => {
            return data;
        });

        // Then
        verify(gateway.findByName(product.name)).once();
        verify(gateway.findByCode(product.code)).never();
    });

    it("Should get a product by description", async () => {
        // Given
        let product: IProduct = newProductInserted(newProduct(), "P-001");

        // Prepare
        when(gateway.findByDescription(product.description)).thenResolve(product);
        
        // When
        await productService.getByDescription(product.description).then((data: IProduct) => {
            return data;
        });

        // Then
        verify(gateway.findByDescription(product.description)).once();
        verify(gateway.findByCode(product.code)).never();
    });
