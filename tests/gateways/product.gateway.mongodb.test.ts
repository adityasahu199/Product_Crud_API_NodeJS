...
describe("Products mongo gateway", async () => {
    ...
    it("Should create a product sucessfully", async () => {
        ...
        expect("good product").to.be.eq(productCreated.description);
    ...
    it("Should update a product sucessfully", async () => {
        ...
        expect("good product").to.be.eq(productUpdated.description);
    ...
    it("Should get a product by its code sucessfully", async () => {
        ...
        expect("good product").to.be.eq(product.description);
        expect(10.50).to.be.eq(product.price);
        expect(2).to.be.eq(product.quantity);
    });

    it("Should get a product by its name sucessfully", async () => {
        // Prepare
        await gateway.create(newProduct(new Date(), "1")).then((data: IProduct) => { });

        // When
        let product: IProduct = await gateway.findByName("Product 01").then((data: IProduct) => {
            return data;
        });
        ...
        expect("1").to.be.eq(product.code);
        expect("Product 01").to.be.eq(product.name);
        expect("product is awesome").to.be.eq(product.description);
        expect(10.50).to.be.eq(product.price);
        expect(2).to.be.eq(product.quantity);
    });

    it("Should get a product by its description sucessfully", async () => {
        // Prepare
        await gateway.create(newProduct(new Date(), "1")).then((data: IProduct) => { });

        // When
        let product: IProduct = await gateway.findByDescription("nice product specs").then((data: IProduct) => {
            return data;
        });

        // Then
        expect("1").to.be.eq(product.code);
        expect("Product 02").to.be.eq(product.name);
        expect("nice product specs").to.be.eq(product.description);
        expect(10.50).to.be.eq(product.price);
        expect(2).to.be.eq(product.quantity);
    });
